"""
btcaaron.network.mempool - Mempool.space provider
"""
...
class MempoolProvider(Provider):
    ...
    def get_fee_estimate(self, target_blocks: int = 6) -> float:
        try:
            response = requests.get(
                f"{self.base_url}/v1/fees/recommended",
                timeout=TIMEOUT
            )
            if response.status_code == 200:
                data = response.json()
                if target_blocks <= 1:
                    return data.get("fastestFee", 10)
                elif target_blocks <= 3:
                    return data.get("halfHourFee", 5)
                else:
                    return data.get("hourFee", 2)
        except Exception:
            pass
        return 5.0
